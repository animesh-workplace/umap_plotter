...
const applyFilters = () => {
	...
	const activeCellTypes = umapCellType.value.filter((ct) => ct.active).map((ct) => ct.name)
...
const graph_options = ref({
	...
	tooltip: {
		...
		formatter: function (params) {
			const expressionText = params.value[3] !== 0 ? `<br/>Expression: ${params.value[3].toFixed(2)}` : ''
	...
	<div class="w-full">
		...
		<div class="flex gap-2 items-center justify-between">
			...
			<div class="flex gap-2 items-center">
				...
				<Icon class="w-5 h-5 text-slate-500" name="akar-icons:settings-horizontal" />
				<div>Filter Mode</div>
			</div>
		</div>

		<div v-if="activateFilterMode" class="flex gap-3 items-center mt-2 justify-center">
			<motion.div :index="index" :while-hover="{ scale: 0.97 }" v-for="(celltype, index) in umapCellType">
				<Tag
					rounded
					:value="celltype.name"
				...
				>
					<template #icon>
						<Icon v-if="celltype.active" class="w-4 h-4 text-rose-500" name="akar-icons:tetragon" />
						<Icon v-else class="w-4 h-4 text-green-500" name="akar-icons:tetragon-fill" />
					</template>
				</Tag>
			</motion.div>
		</div>
