...
const graph_options = ref({
  ...
  tooltip: {
    ...
    formatter: function (params) {
      const expressionText = params.value[3] !== undefined ?
        `<br/>Expression: ${params.value[3].toFixed(2)}` : '';
      return `X: ${params.value[0].toFixed(2)}<br/>Y: ${params.value[1].toFixed(2)}<br/>Cell: ${params.value[2]}${expressionText}`;
    },
    borderColor: '#fff',
  },
  grid: {
  ...
  series: [{ type: 'scatter', data: processedData.value, symbolSize: 5 }],
})



// Watch for store changes to update chart
watch(() => umapStore.geneExpression, () => {
